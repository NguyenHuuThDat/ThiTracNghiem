...
$(document).ready(function () {
  ...
  //       $("body").append($a);
  //       $a.attr("download", "Danh sách sinh viên.xlsx");
  //       $a[0].click();
  //       $a.remove();
  //     },
  //   });
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.status) {
          ...
          const filename = response.filename || "Danh sách sinh viên.xlsx"; // lấy tên file từ response nếu có, không thì dùng mặc định
          const blob = base64ToBlob(base64Data, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'); // thêm type file
          ...
          // Xử lý lỗi nếu response.status là false
      ...
      error: function (error) {
          ...
          alert("Có lỗi xảy ra trong quá trình tải xuống. Vui lòng thử lại.");
      }
    });
  });
