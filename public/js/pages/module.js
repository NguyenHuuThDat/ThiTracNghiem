    ...
    function showGroup(list) {
        ...
        if (list.length == 0) {
            ...
            list.forEach((item, index) => {
                let htmlbtnhide = mode == 1 ? 
                    `<button data-index="${index}" type="button" class="btn btn-alt-secondary btn-sm btn-hide-all ms-2" data-bs-toggle="tooltip" data-bs-placement="top" title="Ẩn tất cả">
                        <i class="far fa-eye-slash"></i>
                    </button>` 
                    : 
                    `<button data-index="${index}" type="button" class="btn btn-alt-secondary btn-sm btn-unhide-all ms-2" data-bs-toggle="tooltip" data-bs-placement="top" title="Huỷ tất cả">
                        <i class="fa fa-rotate-left"></i>
                    </button>`
                html += 
                    `<div>
                        <div class="heading-group d-flex align-items-center">
                            <h2 class="content-heading pb-0" id="${d++}">${"<span class='mamonhoc'>" + item.mamonhoc + "</span>" + " - " + "<span class='tenmonhoc'>" + item.tenmonhoc + "</span>" + " - Năm Học " + "<span class='namhoc'>" + item.namhoc + " - " + (parseInt(item.namhoc) + 1) + "</span>" + " - Học Kỳ " + "<span class='hocky'>" + item.hocky + "</span>"}</h2>
                            ${htmlbtnhide}
                    </div>
                ...
                item.nhom.forEach((nhom_item) => {
                    ...
                    if (nhom_item.hienthi == 1) {
                        btn_hide = 
                            `<a class="nav-main-link dropdown-item btn-hide-group" href="javascript:void(0)" data-id="${nhom_item.manhom}">
                                <i class="nav-main-link-icon si si-eye me-2 text-dark"></i>
                                <span class="nav-main-link-name fw-normal">Ẩn nhóm</span>
                            </a>`
                    } else {
                        btn_hide = 
                            `<a class="nav-main-link dropdown-item btn-unhide-group" href="javascript:void(0)" data-id="${nhom_item.manhom}">
