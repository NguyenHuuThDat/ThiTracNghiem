                            ...
                            <div class="d-md-flex justify-content-md-between align-items-md-center">
                                <div class="p-1 p-md-3">
                                    ...
                                    <p class="fs-sm text-muted mb-2">
                                        <i class="fa fa-layer-group me-1"></i> Gửi cho học phần <strong data-bs-toggle="tooltip" data-bs-animation="true" data-bs-placement="top" style="cursor:pointer" data-bs-original-title="${announce.nhom}">${announce.tenmonhoc} - Năm Học ${announce.namhoc} - ${Number(announce.namhoc) + 1} - HK${announce.hocky}</strong>
                                    </p>
                                </div>
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.forEach((item, index) => {
                    html += `<option value="${index}">${item.mamonhoc + " - " + item.tenmonhoc + " - Năm Học " + item.namhoc + " - " + (Number(item.namhoc) + 1) + " - Học Kỳ " + item.hocky}</option>`;
                });
                $("#nhom-hp").html(html);
            }
        });
        ...
        if (groups[index].nhom.length > 0) {
            ...
            groups[index].nhom.forEach(item => {
                html += `<div class="col-4">
                            <div class="form-check">
                                <input class="form-check-input select-group-item" type="checkbox" value="${item.manhom}" id="nhom-${item.manhom}" name="nhom-${item.manhom}">
                                <label class="form-check-label" for="nhom-${item.manhom}">${item.tennhom}</label>
                            </div>
                        </div>`
            });
        } else {
            html += `<div class="text-center fs-sm">
